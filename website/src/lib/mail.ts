...
const FROM = process.env.MAIL_FROM ?? 'Nocrave <hello@joincurb.app>';
 ...
 *   so the mark carries `alt="Nocrave"` with explicit width/height: blocked, it
 ...
 *
 * And it says nothing about what Nocrave is for. These land in inboxes other
...
export function signInCodeEmail(code: string) {
  const subject = `${code} is your Nocrave code`;

  const text = [
    `Your Nocrave sign-in code is ${code}.`,
    '',
...
<html lang="en">
  <head>
    ...
    <title>Your Nocrave sign-in code</title>
  </head>
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
            ...
            <tr>
              <td style="padding:32px 32px 0 32px;">
                <img src="https://joincurb.app/email-mark.png" alt="Nocrave"
                     width="44" height="44"
          ...
          <div style="max-width:440px;padding:20px 8px 0 8px;
            ...
            Sent by Nocrave because someone entered this address to sign in.
          </div>
        </td>
      </tr>
    </table>
